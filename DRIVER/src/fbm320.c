...
FBMTYPE FBM;
u8 ALTIUDE_OK=0, ALT_Updated=0;
float RPFilter;

//****************************************A
// 1 写1个字节数据到FBM320寄存器
u8 FBM320_WriteByte(uint8_t reg,uint8_t data)
{
	if(IIC_WriteOneByte(FBMAddr2,reg,data))
	   return 1;
	else
	   return 0;
}

// 2 读1个字节数据从FBM320的寄存器中
u8 FBM320_ReadByte(uint8_t reg,uint8_t *buf)
{
	if(IIC_ReadOneByte(FBMAddr2,reg,buf))
	   return 1;
	else
	   return 0;
}
...
{
	if(IIC_WriteLengthBytes(FBMAddr2,reg,len,buf))
	   return 1;
	else
	   return 0;
}
...
// 4 从指定寄存器读取指定长度数据
u8 FBM320_ReadMultBytes(uint8_t reg,uint8_t len,uint8_t *buf)
{
	if(IIC_ReadLengthBytes(FBMAddr2,reg,len,buf))
	   return 1;
	else
	   return 0;
}
...
//****************************************B
// 5 读取FBM320 WHO_AM_I寄存器标识，返回0x42
u8 FBM320_getDeviceID(void)
{
    u8 buf;
	FBM320_ReadByte(FBM_ID, &buf); //地址在0x6B，默认值为0x42
    printf("Value of FBM_ID: %d", buf);
	return buf;
}

//****************************************C
// 6 检测FBM320是否已经连接
u8 FBM320_testConnection(void) 
{
...
//****************************************D
// 8 初始化FB320并获取FBM320的校准数据
void FBM320_Init(void)
{
	FBM320_Check(); //检查FBM320与MCU是否正常通信 Function 7
...
uint8_t Init_Altitude(void)
{
	...
	if(GET_FLAG(BAR_OFFSET)) //INIT_ALTIUDE_OK=1 初始化开始
	{
		...
		if(cnt_p == 100)
		{
			...
			SENSOR_FLAG_RESET(BAR_OFFSET);//校准气压计结束
			cnt_p = 0; 
			return 1;
		}
		cnt_p++;
...
//		Aver_Filter(FBM.RP,&FBM.RPFilter,5); //滑动窗口滤波//滤波有点问题!!!!!
	...
	//FBM.Altitude =  ((pow((1015.7f / (FBM.RP/100)), 0.1902630958) - 1.0f) * (25 + 273.15f)) / 0.0065f;
		...
		SortAver_Filter(FBM.Altitude ,&AltitudeFilter,12); //去极值均值滤波
		if(AltitudeFilter)
		Aver_Filter(AltitudeFilter,&FBM.AltitudeFilter,8); //滑动窗口滤波
