   ...
   //遥控器K1按键
	 ...
	 if((PreButton&0x02) != (Button&0x02)) 
	 {
		 if(Button&0x02)//打开WiFi  
		 {
			 ...
			 WiFi_Switch(ENABLE);
			 Run_flag = 0;
			 SENSOR_FLAG_SET(WiFi_ONOFF); //开启WiFi
			 WiFi_LEDflag = 1;
		 ...
		 else //关闭WiFi
		 {
			 WiFi_Switch(DISABLE);
			 Run_flag = 0;
			 SENSOR_FLAG_RESET(WiFi_ONOFF); //关闭WiFi
			 WiFi_LEDflag = 2; 
		 }
	}
	 //遥控器K3按键
	 ...
	 if((PreButton&0x08) != (Button&0x08))
	 {
		 if(Button&0x08)
		 {
			 SENSOR_FLAG_SET(FLY_MODE); //无头模式
		 }else 
		 {
			 SENSOR_FLAG_RESET(FLY_MODE); //有头模式
		 }
	 }
	 PreButton = Button;
}
...
{
	//uint16_t num,i;
//	uint8_t colour[3],led; //OneNET变量，没用到
...
//		num +=buff[i];
//	}
//	if((num&0xff)!= buff[cnt]) return; //判断校验 若需要则直接取消注释即可
//	else 
			  ...
			  switch(buff[4])
				{
					/* 陀螺仪校准 */
					case 0x01:           
						SENSOR_FLAG_SET(GYRO_OFFSET); 	break;  
					
					/* 加速度校准 */
					case 0x02:
						SENSOR_FLAG_SET(ACC_OFFSET); 	break;  
					...
					/* 气压计校准 */					
					case 0x04:
						SENSOR_FLAG_SET(BAR_OFFSET);	break;  
...
void NRF_SingalCheck(void)
{
	...
	if(!WiFi_Controlflag)
	{
		...
		{
			if(Airplane_Enable && !RGB_BATflag)//飞机遥控连接正常
			{
				RGB_Fly(); //飞行指示灯
			}
			PreDataID = DataID;
...
void SendToRemote(void)
{
	...
	if(Airplane_Enable)
	{
		SENSOR_FLAG_SET(FLY_ENABLE); //解锁模式置位
	}
	else
	{
		SENSOR_FLAG_RESET(FLY_ENABLE); //上锁模式复位
	}
	NRF_TX_DATA[0] = 0xFF;//帧头
	
	NRF_TX_DATA[1] = SENSOR_OFFSET_FLAG; //标志位组
