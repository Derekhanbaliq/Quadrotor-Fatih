...
#define RadtoDeg    57.324841f	//相当于在imu.c和ANO_DT.c重复定义了
...
/////////////////////////////////////////////////////////////////////////////////////
	if(f.send_senser)
	{
		...
		ANO_DT_Send_Senser((s16)Acc_filt.X,(s16)Acc_filt.Y,(s16)Acc_filt.Z,
											 (s16)Gyro_rad.X*RadtoDeg,(s16)Gyro_rad.Y*RadtoDeg,(s16)Gyro_rad.Z*RadtoDeg,0,0,0,0);
//		#if defined (ROL_PID_DEBUG)   //ROLL角调试
//			Data_Send_AngleRate(Gyro_rad.X*RadtoDeg,PID_ROL_Rate.Pout,PID_ROL_Rate.Iout,PID_ROL_Rate.Dout,
//		                       PID_ROL_Angle.Error,PID_ROL_Angle.Pout,PID_ROL_Angle.Iout,PID_ROL_Rate.OutPut);
//		
//		#elif defined (PIT_PID_DEBUG) //PITCH角调试	
//			Data_Send_AngleRate(Gyro_rad.Y*RadtoDeg,PID_PIT_Rate.Pout,PID_PIT_Rate.Iout,PID_PIT_Rate.Dout,
//		                       Target_Angle.pit,Measure_Angle.pit,PID_PIT_Angle.OutPut,PID_PIT_Rate.OutPut);
//		
//		#elif defined (YAW_PID_DEBUG) //YAW角调试
//			Data_Send_AngleRate(Gyro_rad.Z*RadtoDeg,PID_YAW_Rate.Pout,PID_YAW_Rate.Iout,PID_YAW_Rate.Dout,
...
void ANO_DT_Data_Receive_Anl(uint8_t *data_buf,uint8_t num)
{
	...
	if(*(data_buf+2)==0X12)								//PID3 恢复默认参数
	{	
		ANO_DT_Send_Check(*(data_buf+2),sum);
	}
